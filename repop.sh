...
PROF_ID=$(curl -X POST -H "Content-Type: application/json" http://localhost:5000/admin/staff -d '{"email": "akshay@fm.com", "password": "12345678", "name": "Akshay Narayan"}' | jq '.ID')

MODULE_ID=$(curl -X POST -H "Content-Type: application/json" http://localhost:5000/module -d '{"code": "CS2103", "name":"Software Engineering", "semester": "AY2021/2022 1"}' | jq '.ID')
curl -X POST -H "Content-Type: application/json" http://localhost:5000/module/supervise -d '{"moduleId": '"$MODULE_ID"', "staffId":'"$PROF_ID"'}'
...
curl -X POST -H "Content-Type: application/json" http://localhost:5000/module/enroll -d '{"moduleId": '"$MODULE_ID"', "studentId":'"$STUDENT_ID"'}'
